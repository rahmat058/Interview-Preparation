    ...
    <aside className="space-y-5" aria-label="Catalog filters">
      ...
      <FilterSection title="Sort By">
        <p className="mb-2 text-xs text-slate-500">
          Current:{' '}
          <span className="font-medium text-teal-700">{getSortLabel(sortBy)}</span>
        </p>
        <SortSelect />
      </FilterSection>
      ...
      <p className="text-xs text-slate-500">
        <span className="font-semibold text-teal-700">{resultCount}</span> product
        {resultCount === 1 ? '' : 's'} · sorted by{' '}
        <span className="font-medium text-slate-600">{getSortLabel(sortBy)}</span>
      </p>
    </aside>
  )
}
