...
export function FeedList() {
  ...
  if (isFailed && posts.length === 0) {
    return <FeedError message={error ?? 'Failed to load feed'} onRetry={handleRetry} isRetrying={isLoading} />
  }
  ...
  return (
    <div className="feed-scroll-anchor space-y-4">
      {posts.map((post) => {
        ...
        return <FeedCard key={post.id} post={post} isNew={isNew} staggerIndex={staggerIndex} />
      })}

      {isFailed && posts.length > 0 && (
        <FeedError message={error ?? 'Failed to load more'} onRetry={handleRetry} isRetrying={isLoadingMore} />
      )}
